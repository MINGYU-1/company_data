...
* 데이터 로드 및 점검
	•	이상치제거한_일반음식점.csv 파일을 UTF-8 → CP949 순으로 로드.
	•	전처리된 파일이기 때문에 결측/이상치 재점검은 최소화하고 바로 폐업률 분석에 집중.

* 면적 4분위(Quartile)에 따른 폐업률 분석
	•	qcut으로 소재지면적을 4개 등분위 구간으로 재분류.
	•	구간 라벨:
하위 25% / 2550% / 5075% / 상위 75%
	...
	•	폐업 business count
	•	폐업률(%) 산출
	•	결과는 폐업률 기준 내림차순 정렬하여 어떤 면적대가 폐업 위험이 높은지 바로 확인 가능.
	•	분석 결과 CSV 저장:
면적_범위별_폐업률.csv

* 시각화 — Bar Chart
	•	폰트 한글 설정 후 seaborn barplot 사용.
	•	각 bar 위에 폐업률을 소수점 둘째 자리까지 표시.
	•	파일 저장:
소재지면적별_폐업률_비교.png
...
* 데이터 정리
	•	파일 로드(CP949 → UTF-8 fallback).
	•	전체 32개 컬럼 중 전부 null인 컬럼 완전 삭제.
	•	이후 자본금 컬럼도 분석과 무관하여 제거.
	•	구조(info) 재확인 후 불필요 열 없는 상태로 정돈.

* 폐업일자 기준 사업장 분포 분석
	•	전체 row 수 = N
	•	폐업일자 NULL vs NOT NULL 개수 집계.
	•	pie chart 표시를 위해 비율 + 실제 건수 병기 함수 제작
...
**폴더 경로 탐색 및 파일 확인**
	•	Path.cwd() → 현재 작업 디렉토리
	•	.parent → 상위 디렉토리
	•	상위 폴더 안의 /second_week/data_csv 경로 자동 생성
	•	해당 폴더에서 *.csv 파일 리스트를 전부 수집
→ 발견된 CSV 개수 출력
...
각 CSV 파일별로 다음 정보 저장:
	•	최종 리스트를 DataFrame으로 묶어서 csv_column_summary.csv 로 저장 (UTF-8 인코딩)
...
* summary_df[‘Columns’] 전체 순회
	•	각 파일에서 수집한 컬럼 구조를 반복문으로 검사
	•	추출된 컬럼명 구조가 올바른지 모니터링

* 폴더 내 CSV 목록 재확인
	•	second_week/data_csv 경로를 다시 점검
	•	CSV 파일 리스트 개수 출력
	•	누락 파일 혹은 불러오지 못한 파일 여부 확인
...
폐업 여부(Y=폐업여부)를 예측하는 Baseline Logistic Regression 모델을 구축했다.

⸻

* **데이터 로드 및 기본 전처리**
	•	CP949 인코딩으로 로드
	•	폐업일자가 존재하면 폐업(1), 없으면 정상(0) → 폐업여부 컬럼 생성
	•	소재지면적 결측치는 **중앙값(median)**으로 대체
	•	위생업태명은 One-hot 인코딩(dummies), 첫 범주는 drop_first
