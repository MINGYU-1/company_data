...
---
## 1주차 정리
네, 11월 9일부터 11월 14일까지의 모든 작업 내역을 날짜순으로 정리했습니다.

---

## 📅 11월 9일: 프로젝트 기획 및 데이터 수집

* **프로젝트 기획**: 소상공인(SME) 데이터를 활용한 분석 아이디어를 구상하고, 분석할 업종을 선정했습니다.
* **Git 학습**: Git의 `merge` 기능 사용법을 확인했습니다.
* **데이터 수집 스크립트 작성**: Wehago API(`data.go.kr`)에서 데이터를 대량 수집하는 Python 스크립트를 작성했습니다.
    * `requests`와 `dotenv`를 사용해 API 키를 관리했습니다.
    * `DATASET_SUNDO_69` (숙박업) 데이터를 500개 단위로 반복 조회하여 전체를 가져오는 로직을 구현했습니다.
    * 수집한 데이터를 `data_csv/숙박업_강원도_읍면동.csv` 파일로 저장했습니다.
